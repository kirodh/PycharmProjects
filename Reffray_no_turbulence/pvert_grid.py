...
# how to use:
# pvert_grid(data_depth_values, plot_graph_of_spacing,font_size, line_thickness, horizontal_line, horizontal_line_thickness, y_limits, x_limits)
...
def pvert_grid(data,spacing=0,font = 35,thickness = 2.5,horz = 0,horz_value = None,ylim=None,xlim=None):
    ...
    num_levels = np.array(num_levels) # make it a numpy array
    ...
    plt.title("Depth at level",fontsize = font) # label titles
    ...
    if spacing:
        ...
        plt.title("Spacing between levels",fontsize = font) # label titles
        plt.xlabel("number of levels",fontsize = font)
